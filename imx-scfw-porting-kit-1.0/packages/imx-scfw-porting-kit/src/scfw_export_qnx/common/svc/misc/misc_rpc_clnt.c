...
#include <sys/svc/rm/api.h>
#include <sys/svc/misc/api.h>
#include "rpc.h"
...
sc_err_t sc_misc_seco_forward_lifecycle(sc_ipc_t ipc, uint32_t change)
{
    ...
    RPC_U32(&msg, 0U) = (uint32_t) change;
    RPC_SIZE(&msg) = 2U;
...
sc_err_t sc_misc_seco_attest_mode(sc_ipc_t ipc, uint32_t mode)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_ATTEST_MODE;
    RPC_U32(&msg, 0U) = (uint32_t) mode;
    RPC_SIZE(&msg) = 2U;
...
sc_err_t sc_misc_seco_attest(sc_ipc_t ipc, uint64_t nonce)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_ATTEST;
    RPC_U32(&msg, 0U) = (uint32_t) (nonce >> 32U);
    RPC_U32(&msg, 4U) = (uint32_t) nonce;
    RPC_SIZE(&msg) = 3U;
...
sc_err_t sc_misc_seco_get_attest_pkey(sc_ipc_t ipc, sc_faddr_t addr)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_GET_ATTEST_PKEY;
    RPC_U32(&msg, 0U) = (uint32_t) (addr >> 32U);
    RPC_U32(&msg, 4U) = (uint32_t) addr;
    RPC_SIZE(&msg) = 3U;
...
sc_err_t sc_misc_seco_get_attest_sign(sc_ipc_t ipc, sc_faddr_t addr)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_GET_ATTEST_SIGN;
    RPC_U32(&msg, 0U) = (uint32_t) (addr >> 32U);
    RPC_U32(&msg, 4U) = (uint32_t) addr;
    RPC_SIZE(&msg) = 3U;
...
sc_err_t sc_misc_seco_attest_verify(sc_ipc_t ipc, sc_faddr_t addr)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_ATTEST_VERIFY;
    RPC_U32(&msg, 0U) = (uint32_t) (addr >> 32U);
    RPC_U32(&msg, 4U) = (uint32_t) addr;
    RPC_SIZE(&msg) = 3U;
...
sc_err_t sc_misc_seco_commit(sc_ipc_t ipc, uint32_t *info)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_SECO_COMMIT;
    RPC_U32(&msg, 0U) = *(uint32_t*) info;
    RPC_SIZE(&msg) = 2U;
...
sc_err_t sc_misc_get_boot_type(sc_ipc_t ipc, sc_misc_bt_t *type)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_GET_BOOT_TYPE;
    RPC_SIZE(&msg) = 1U;
...
sc_err_t sc_misc_rompatch_checksum(sc_ipc_t ipc, uint32_t *checksum)
{
    ...
    RPC_VER(&msg) = SC_RPC_VERSION;
    RPC_SVC(&msg) = (uint8_t) SC_RPC_SVC_MISC;
    RPC_FUNC(&msg) = (uint8_t) MISC_FUNC_ROMPATCH_CHECKSUM;
    RPC_SIZE(&msg) = 1U;
